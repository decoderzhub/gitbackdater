...
st.write(f"Current repository path: {st.session_state['repo_path']}")

github_repo_url = st.text_input("Enter Github repository URL:", value=st.session_state['github_repo_url'], key='github_repo_url')
st.write(f"Current Github repository URL: {st.session_state['github_repo_url']}")

commit_date = st.date_input("Select date for commit:", value=st.session_state['commit_date'], key='commit_date')
st.write(f"Current commit date: {st.session_state['commit_date']}")

commit_number = st.number_input("Number of commits:", min_value=1, value=st.session_state['commit_number'], step=1)
st.write(f"Current number of commits: {st.session_state['commit_number']}")
...
if st.button("Make Commit"):
    ...
    for i in range(commit_number):
        # Create a file
        touch_command = f"touch {os.path.join(repo_path, st.session_state['commit_data'][i]['file_name'])} && echo 'hello world!' >> {os.path.join(repo_path, st.session_state['commit_data'][i]['file_name'])}"
        # Add file to git
        ...
        try:
            ...
            with st.spinner('Creating file...'):
                ...
                time.sleep(2)
            with st.spinner('Adding file to git...'):
            ...
            with st.spinner('Creating commit...'):
                ...
                time.sleep(2)
            with st.spinner('Amending commit date and time...'):
                amend_result = subprocess.run(git_amend_command,shell=True, check=False, text=True, capture_output=False)
        ...
        try:
            with st.spinner('Pushing to remote repository...'):
